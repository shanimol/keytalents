...
export default function TeamsPage() {
  ...
  const handleSubmit = async (data: TeamCreate) => {
    try {
      setFormError(null)
      if (editTarget) {
        await updateTeam({ id: editTarget.id, body: data }).unwrap()
      } else {
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center text-sm text-gray-400">
                  {searchParams.get("search") ? "No teams match your search." : "No teams yet. Add your first team."}
                </td>
              </tr>
            ) : (
              data?.items.map((team) => (
        ...
        {data && data.total > 0 && (
          <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
            <p className="text-sm text-gray-500">
              ...
              <span className="font-medium text-gray-900">{data.total}</span> teams
            </p>
